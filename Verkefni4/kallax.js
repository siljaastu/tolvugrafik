...
function render()
{
    ...
    mv1 = mult( mv1, scalem( h+2*t, 2*t, d ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );
    ...
    mv1 = mult( mv1, scalem( h+t, t, d-0.01 ) ); //the dividers are a tiny bit smaller
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );

    // middle shelf vertic
    mv1 = mult( mv, translate( 0.0, 0.0, 0.0 ) );
    mv1 = mult( mv1, scalem( t, h+t, d-0.02 ) ); //the dividers are a tiny bit smaller (vert more)
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mv1));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );
